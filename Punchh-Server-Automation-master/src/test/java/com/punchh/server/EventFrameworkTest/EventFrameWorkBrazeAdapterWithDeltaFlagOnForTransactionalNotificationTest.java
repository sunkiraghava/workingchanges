...
public class EventFrameWorkBrazeAdapterWithDeltaFlagOnForTransactionalNotificationTest {
	static Logger logger = LogManager
			.getLogger(EventFrameWorkBrazeAdapterWithDeltaFlagOnForTransactionalNotificationTest.class);
	...
	@Test(description = "SQ-T6098 [Outbound - Braze] Triggering of Transactional Notifications Event by enabling the Delta Flag")
	public void T6098_VerifyBrazeBasedTriggeringEventWithEnablingDeltaFlagForTransactionalNotificationsEvent()
			throws Exception {
	...
	private void processEventType(String eventTypeName, String adapterName, String devBootstrapServers)
		...
		if (!eventType.equalsIgnoreCase("user_signup")) {

			Assert.assertFalse(dispatchedPayload.contains("\"attributes\":["),
					"Key value mismatch found in the payload: attributes");
			logger.info("Verified that attributes key is not coming for the event : " + eventType);
			TestListeners.extentTest.get()
					.pass("Verified that attributes key is not coming for the event : " + eventType);

			Assert.assertTrue(dispatchedPayload.contains("\"events\":["),
					"Key value mismatch found in the payload:events ");
			logger.info("Verified that events key is coming for the event : " + eventType);
			TestListeners.extentTest.get().pass("Verified that events key is coming for the event : " + eventType);

		} else if (eventType.equalsIgnoreCase("user_signup")) {
			Assert.assertTrue(dispatchedPayload.contains("\"attributes\":["),
					"Key value mismatch found in the payload: attributes");
			logger.info("Verified that attributes key is coming for the event : " + eventType);
			TestListeners.extentTest.get().pass("Verified that attributes key is coming for the event : " + eventType);

			Assert.assertFalse(dispatchedPayload.contains("\"events\":["),
					"Key value mismatch found in the payload: events");

			logger.info("Verified that events key is not coming for the event : " + eventType);
			TestListeners.extentTest.get().pass("Verified that events key is not coming for the event : " + eventType);

...
public class EventFrameWorkBrazeAdapterWithDeltaFlagOnTest {
	static Logger logger = LogManager.getLogger(EventFrameWorkBrazeAdapterWithDeltaFlagOnTest.class);
	...
	private static final Map<String, String> METHODNAME_PAYLOAD = Map.ofEntries(
			...
			Map.entry("gift_checkin", "getBrazeGiftCheckinpayload"),
			Map.entry("rewards", "getBrazeRewardPayload"),
	...
	@Test(description = "(SQ-T6094) [Outbound - Braze] Triggering of Guest Update Event by enabling the Delta Flag"
			+ "(SQ-T6093) [Outbound - Braze] Triggering of Guest Create & Delete Event by enabling the Delta Flag"
			+ "(SQ-T6095) [Outbound - Braze] Triggering of Marketing Notification Event by enabling the Delta Flag"
			+ "(SQ-T6097) [Outbound - Braze] Triggering of Checkin, Reward & Redemptions Events by enabling the Delta Flag")
	public void T6094_VerifyBrazeBasedTriggeringEventWithEnablingDeltaFlag() throws Exception {
	...
	private void processEventType(String eventType, String adapterName, String devBootstrapServers) throws Exception {
		...
		if (!previousChangesKeysList.isEmpty() || previousChangesKeysList.size() != 0) {
			...
			eventsPayloadDetailsMap.put("attributes", "");
			String resultString = utils.verifiedKeyValueDispatchedEventPayload(eventsPayloadDetailsMap,
					dispatchedPayload, keysToSkipList);
			
			Assert.assertTrue(resultString.contains("attributes"), "Key value mismatch found in the payload: " + resultString);

...
public class GiftCardLegacyPaymentTest {

	private static Logger logger = LogManager.getLogger(GiftCardLegacyPaymentTest.class);
	...
	@Test(description = "SQ-T2788 INTD-396 | Failure Gift Card/Payment logs | Legacy Payment", groups = { "regression",
			"dailyrun" }, priority = 0)
	@Owner(name = "Hardik Bhardwaj")
	public void T2788_failureGiftCardOrPaymentLogs_LegacyPayment_PartOne_ReloadGiftCard() throws InterruptedException {
...
//		pageObj.instanceDashboardPage().loginToInstance();
		...
		pageObj.giftcardsPage().braintreeCredentials(dataSet.get("invalidPublicKey"));
		// Reload Gift Card API2
		String amount1 = utils.twoDigitDecimalNumberUnderFiveHundred();
		logger.info("Amount used in reload gift card " + amount1);
		utils.logit("Amount used in reload gift card " + amount1);
		...
		pageObj.instanceDashboardPage().transactionTypeAndTime("Payments");
		boolean flag = pageObj.giftcardsPage().checkGiftCardLogs(firstName, amount1);
		Assert.assertTrue(flag, "Gift card Logs is not found");
		logger.info("Gift card Logs is found");
		utils.logPass("Gift card Logs is found");
	...
	@Test(description = "SQ-T2788 INTD-396 | Failure Gift Card/Payment logs | Legacy Payment", groups = { "regression",
			"dailyrun" }, priority = 1)
	@Owner(name = "Hardik Bhardwaj")
	public void T2788_failureGiftCardOrPaymentLogs_LegacyPayment_PartTwo_GiftCardPurchase()
...
//		pageObj.instanceDashboardPage().loginToInstance();
		...
		pageObj.menupage().navigateToSubMenuItem("Support", "Gift Card Log");
		pageObj.instanceDashboardPage().transactionTypeAndTime("Payments");
		boolean flag = pageObj.giftcardsPage().checkGiftCardLogs(firstName, amount);
		Assert.assertTrue(flag, "Gift card Logs is not found");
		logger.info("Gift card Logs is found");
		utils.logPass("Gift card Logs is found");
	}
	...
	@Test(description = "SQ-T2788 INTD-396 | Failure Gift Card/Payment logs | Legacy Payment", groups = { "regression",
			"dailyrun" }, priority = 2)
	@Owner(name = "Hardik Bhardwaj")
	public void T2788_failureGiftCardOrPaymentLogs_LegacyPayment_PartThree_GiftAGiftCard() throws InterruptedException {
		// User SignUp using API
		String userEmail = pageObj.iframeSingUpPage().generateEmail();
		Response signUpResponse = pageObj.endpoints().Api2SignUp(userEmail, dataSet.get("client"),
				dataSet.get("secret"));
...
//		pageObj.instanceDashboardPage().loginToInstance();
		...
		pageObj.menupage().navigateToSubMenuItem("Support", "Gift Card Log");
		pageObj.instanceDashboardPage().transactionTypeAndTime("Payments");
		boolean flag = pageObj.giftcardsPage().checkGiftCardLogs(firstName1, amount);
		Assert.assertTrue(flag, "Gift card Logs is not found");
		logger.info("Gift card Logs is found");
		utils.logPass("Gift card Logs is found");
	}

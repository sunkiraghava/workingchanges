...
public class ValidationInBatchRedemptionProcessOMM_T146 {
	static Logger logger = LogManager.getLogger(ValidationInBatchRedemptionProcessOMM_T146.class);
	...
	private String lisExternalID, qcExternalID, redeemableExternalID,qcExternalID2, redeemableExternalID2, businessID, lisExternalID2,lisExternalID4, lisExternalID3;
	...
	@Test(description = "SQ-T3447 Step 1 : Verify cases for Item Qualifiers expression 'Line Item Does Not Exist'", priority = 0)
	@Owner(name = "Ashwini Shetty")
	public void verify146_1() throws Exception {
		...
		// ===================== Create LIS =====================
		String lisName = "AutoSelect_LIS_T146_" + Utilities.getTimestamp();
		...
		// ===================== Create LIS 2 =====================
		String lisName2 = "AutoSelect_LIS_2_T146_" + Utilities.getTimestamp();
		...
		// ===================== Create LIS 3 =====================
		String lisName3 = "AutoSelect_LIS_3_T146_" + Utilities.getTimestamp();
		String lisPayload3 = apipayloadObj.lineItemSelectorBuilder().setName(lisName3)
				.setFilterItemSet(LineItemSelectorFilterItemSet.BASE_ONLY)
				.addBaseItemClause("item_id", "in", "3419").build();
		Response response3 = pageObj.endpoints().createLIS(lisPayload3, dataSet.get("apiKey"));
		Assert.assertEquals(response3.getStatusCode(), ApiConstants.HTTP_STATUS_OK,
				"Status code 200 did not match for createLIS API");
		utils.logit("LIS Created: " + response3.prettyPrint());
		lisExternalID3 = response3.jsonPath().getString("results[0].external_id");
		utils.logit(" LIS created with External ID: " + lisExternalID3);
		
		// =====================️ Create QC =====================
		String qcname = "AutoSelect_QC_T146_" + Utilities.getTimestamp();
		String qcPayload = apipayloadObj.qualificationCriteriaBuilder().setName(qcname)
				...
				.addItemQualifier("line_item_does_not_exist",lisExternalID2, 0.0, 0)
		        .addItemQualifier("line_item_does_not_exist",lisExternalID3, 0.0, 0).build();
		...
		// ===================== Create Redeemable =====================
		String redeemableName = "AutoRedeemable_T146_"
		...
		addDetails.accept("Pizza2", new String[] { "Pizza2", "1", "7", "M", "10", "999", "2", "201" });
		addDetails.accept("Pizza2", new String[] { "Pizza2", "1", "3", "M", "10", "999", "3", "201" });
	...
	@Test(description = "SQ-T3447 Step 2 : Verify cases for Item Qualifiers expression 'Line Item Does Not Exist'", priority = 1)
	@Owner(name = "Ashwini Shetty")
	public void verify146_2() throws Exception {
		...
		Assert.assertEquals(actualRedeemption2DiscountAmt, expRedeemption2DiscountAmt, 
		        "Redemption 2 discount amount mismatch");
	...
	@Test(description = "SQ-T3447 Step 3 : Verify cases for Item Qualifiers expression 'Line Item Does Not Exist'", priority = 2)
	@Owner(name = "Ashwini Shetty")
	public void verify146_3() throws Exception {
		...
		// ===================== Create LIS 4 =====================
		String lisName4 = "AutoSelect_LIS_4_T146_" + Utilities.getTimestamp();
		String lisPayload4 = apipayloadObj.lineItemSelectorBuilder().setName(lisName4)
				.setFilterItemSet(LineItemSelectorFilterItemSet.BASE_ONLY)
				.addBaseItemClause("item_id", "in", "2001").build();
		Response response4 = pageObj.endpoints().createLIS(lisPayload4, dataSet.get("apiKey"));
		Assert.assertEquals(response4.getStatusCode(), ApiConstants.HTTP_STATUS_OK,
				"Status code 200 did not match for createLIS API");
		utils.logit("LIS Created: " + response4.prettyPrint());
		lisExternalID4 = response4.jsonPath().getString("results[0].external_id");
		utils.logit("pass", " LIS created with External ID: " + lisExternalID4);
		
		// =====================️ Create QC 2 =====================
		String qcname2 = "AutoSelect_QC_2_T146_" + Utilities.getTimestamp();
		String qcPayload2 = apipayloadObj.qualificationCriteriaBuilder().setName(qcname2)
				.setPercentageOfProcessedAmount(5)
				.setQCProcessingFunction("sum_amounts").setStackDiscounting(true).setReuseQualifyingItems(true)
				.addLineItemFilter(lisExternalID, "", 0)
				.addItemQualifier("line_item_does_not_exist",lisExternalID4, 0.0, 0).build();
		...
		Assert.assertEquals(qcResponse2.getStatusCode(), ApiConstants.HTTP_STATUS_OK,
				"Status code 200 did not match for createLIS API");
		utils.logit("API response: " + qcResponse2.prettyPrint());

		// Get QC External ID
		qcExternalID2 = qcResponse2.jsonPath().getString("results[0].external_id");
		utils.logit("pass", qcname2 + "QC is created with External ID: " + qcExternalID2);

		// ===================== Create Redeemable 2=====================
		String redeemableName2 = "AutoRedeemable_T146_2_"
				+ Utilities.getTimestamp();
		...
		utils.logit("pass", redeemableName2 + " redeemable is Created Redeemable External ID: "
		...
		addDetails.accept("Pizza3", new String[] { "Pizza2", "1", "3", "M", "10", "999", "3", "201" });
		...
		String expErrorMessage = dataSet.get("expErrorMessage");
		utils.logInfo("Expected Redemption Error Message: " + expErrorMessage);
		String actualRedeemption2ErrorMessage = batchRedemptionProcessResponseUser1.jsonPath()
				.getString("failures[0].message").replace("[", "").replace("]", "");
		utils.logInfo("Actual Redemption Error Message 1: " + actualRedeemption2ErrorMessage);
		boolean result = pageObj.apiUtils().verifyErrorMessage(actualRedeemption2ErrorMessage, expErrorMessage);
		Assert.assertTrue(result, "Redemption error message 1 did not match expected");

		String actualRedeemption3ErrorMessage = batchRedemptionProcessResponseUser1.jsonPath()
				.getString("failures[1].message").replace("[", "").replace("]", "");
		utils.logInfo("Actual Redemption Error Message 2: " + actualRedeemption3ErrorMessage);
		boolean result1 = pageObj.apiUtils().verifyErrorMessage(actualRedeemption3ErrorMessage, expErrorMessage);
		Assert.assertTrue(result1, "Redemption error message 2 did not match expected");
	}

	@Test(description = "SQ-T3447 Step 4 : Verify cases for Item Qualifiers expression 'Line Item Does Not Exist'", priority = 3)
	@Owner(name = "Ashwini Shetty")
	public void verify146_4() throws Exception {
		...
		// ===================== Create Redeemable 2 =====================
		String redeemableName2 = "AutoRedeemable_2_T146_"
				+ Utilities.getTimestamp();
		RedeemablePayloadBuilder builder2 = apipayloadObj.redeemableBuilder();
		String redeemablePayload2 = builder2.startNewData().setName(redeemableName2)
				.setBooleanField("available_as_template", false)
		...
		addDetails.accept("Pizza1", new String[] { "Pizza1", "1", "8", "M", "10", "999", "1", "101" });
		addDetails.accept("Pizza2", new String[] { "Pizza2", "1", "7", "M", "10", "999", "2", "201" });
		addDetails.accept("Pizza3", new String[] { "Pizza3", "1", "3", "M", "10", "999", "3", "201" });
		...
		double actualTotalDiscountAmount = actualRedeemption1DiscountAmt + actualRedeemption2DiscountAmt;
		utils.logit("Actual Total Discount Amount: " + actualTotalDiscountAmount);
		Assert.assertEquals(actualTotalDiscountAmount, expTotalDiscountAmount,"Overall total discount amount for the redemption mismatch");
	}
	...
	public void tearDown() throws Exception {
		...
		utils.deleteLISQCRedeemable(env, lisExternalID3, "", "");
		utils.deleteLISQCRedeemable(env, lisExternalID4, "", "");

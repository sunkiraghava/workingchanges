...
public class MigrateduserRedemptionMarkTest_Part2 {
	private static Logger logger = LogManager.getLogger(MigrateduserRedemptionMarkTest_Part2.class);
	...
	@Test(description = "OLF-T1947 [Business having less redemption mark than previous redemption mark] Verify migrated user having exact initial points which is equal to new redemption mark"
			+ "OLF-T1950 [Business having less redemption mark than previous redemption mark] Verify user balance in API's (api/auth/users/balance) for user having exact points which is equal to new redemption mark"
			+ "OLF-T1951 [Business having less redemption mark than previous redemption mark] Verify user balance in API's (api/mobile/checkins/balance) for user having exact points which is equal to new redemption mark. ", priority = 0)
	public void T1947_verifyMigratedUserHavingExactInitialPointsWhichIsEqualToNewRedemptionMark() throws Exception {
		...
		double initialPointsD = Double.parseDouble(dataSet.get("initialPoints"));
		...
		int iFrameActualpointsInt = Integer.parseInt(iFrameActualpoints);
		Assert.assertEquals(iFrameActualpointsInt, expectedredeemablePoints,
		...
		// OLF-T1950)
		// [Business having less redemption mark than previous redemption mark] Verify
		// user balance in API's
		// (api/auth/users/balance) for user having exact points which is equal to new
		// redemption mark
		...
		double api2pointsBalance = userBalanceResponse.jsonPath().getDouble("account_balance.points_balance");

		Assert.assertEquals(api2pointsBalance, initialPointsD,
				"User balance is not correct on api2 fetch user balance");

		// OLF-T1951 [Business having less redemption mark than previous redemption
		// mark] Verify user balance in API's (api/mobile/checkins/balance) for user
		...
		String totalCreditd = userCheckinsBalanceResponse.jsonPath().get("total_credits").toString();
		String totaldebit = userCheckinsBalanceResponse.jsonPath().get("total_debits").toString();

		Assert.assertEquals(totalCreditd, total_credits,
				"total_credits is not correct on api1 fetch user checkins balance");
		Assert.assertEquals(totaldebit, total_debits,
				"total_debits is not correct on api1 fetch user checkins balance");

		// OLF-T1952 [Business having less redemption mark than previous redemption
		// mark] Verify user balance in API's (api2/mobile/checkins/account_balance )
		...
		String api2checkinshPointsBalance = accountBalResponse.jsonPath().get("account_balance_details.points_balance")
				.toString();
		String api2checkinsRedeemablePoints = accountBalResponse.jsonPath()
				.get("account_balance_details.redeemable_points").toString();

	}

	@Test(description = "OLF-T1948 [Business having less redemption mark than previous redemption mark] Verify migrated user having less initial points than new redemption mark", priority = 0)
	public void T1948_verifyMigratedUserHavingExactInitialPointsWhichIsEqualToNewRedemptionMark() throws Exception {

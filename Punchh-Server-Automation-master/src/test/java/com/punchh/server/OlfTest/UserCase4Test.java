...
public class UserCase4Test {
	private static Logger logger = LogManager.getLogger(UserCase4Test.class);
	...
	@Test(description = "OLF-T2074 [New ML redemption mark is less than old] Verify point balance in iframe for users when business configuration has membership level Redemption Mark configured", priority = 0)
	public void T2074_VerifyPointBalanceinIframeForUsersWhenBusinessConfigurationHasMembershipLevelRedemptionMarkConfigured()
			throws Exception {
		...
		logger.info("POS checkin is successful for " + "60" + " points");
		utils.logPass("POS checkin is successful for " + "60" + " points");
		...
		Assert.assertEquals(membershipLevel, "Membership Level Gold", "Membership Levelfor Iframe is not matching");
		utils.logPass("Iframe points and membership level validated");

	}

	@Test(description = "OLF-T2093,OLF-T2094,OLF-T2095,OLF-T2096,OLF-T2097,OLF-T2098,OLF-T2099,OLF-T2100 [New ML redemption mark is less than old] Verify user's print timeline when business configuration has membership level Redemption Mark configured"
			+ "[New ML redemption mark is less than old] Verify user's timeline / pie chart when business configuration has membership level Redemption Mark"
			+ "[New ML redemption mark is less than old] Verify user's account history when business configuration has membership level Redemption Mark configured", priority = 1)
	public void T2093_VerifyUsersPrintTimelineWhenBusinessConfigurationHasMembershipLevelRedemptionMarkConfigured()
			throws Exception {
...
//		// login to instance -> here instance is https://divya.punchh.io
		...
		// Do checkin of 1 point OLF-T2101
		Response checkinResponse1 = pageObj.endpoints().posCheckin(userEmail, locationKey, dataSet.get("amount2"));
		Assert.assertEquals(checkinResponse1.getStatusCode(), ApiConstants.HTTP_STATUS_OK, "Status code 200 did not match with POS Checkin ");
		logger.info("POS checkin is successful for " + dataSet.get("amount2") + " points");
		utils.logPass("POS checkin is successful for " + "60" + " points");

		// (OLF-T2103 fetch details from checkin table
		...
		Assert.assertTrue(printTimelienPageContentes.contains(expectedredeemablePoints + ".0 Redeemed"),
		 ...
		 * api1TotalCredits,"Total Credits did not matched for (api/mobile/users/balance)"
		 * ); Assert.assertEquals(actualredeemablePoints,
		 * api1TotalDebits,"Total Debits did not matched for (api/mobile/users/balance)"
		 * );
		 */
		...
		/*
		 * Assert.assertEquals(actualredeemablePoints,
		 * totalCredit,"total cerdit did not matched for (api/mobile/checkins/balance)"
		 * ); Assert.assertEquals(actualredeemablePoints,
		 * totaldebit,"total debit did not matched for (api/mobile/checkins/balance)");
		 */
		...
		/*
		 * Assert.assertEquals(points_earnedApiFetchCheckin,
		 * actualredeemablePoints,"points earned is not matched for (/api/mobile/checkins)"
		 * ); Assert.assertEquals(points_spentApiFetchCheckin,
		 * actualredeemablePoints,"points_spent is not match for (/api/mobile/checkins)"
		 * );
		 */
		...
		Assert.assertEquals(total_credits, "60.0", "total_credits in accounts table is not matching");
		...
		// OLF-T2104 reward_debit table for user
		...
		// OLF-T2105 membership_level_histories table for user

 ...
 * @brief This class contains API test cases for the cancel update API.
 * @fileName ppccLocationCancelUpdateApiTest.java
 */
...
public class ppccLocationCancelUpdateApiTest {
    static Logger logger = LogManager.getLogger(ppccLocationCancelUpdateApiTest.class);
    ...
    @Test(description =  "SQ-T6584 Verify the flow of cancel update api for a location", groups = { "regression" }, priority = 1)
    public void SQ_T6584_verifyTheFlowOfCancelUpdateAPIForALocation() throws Exception {
       ...
       // getting the policy id
        ...
        // checking audit logs
        logger.info("Checking audit logs for cancel update");
        ...
        Assert.assertEquals(locationListAuditLogsResponse.jsonPath().getString("data[" + lastIndex + "].event_type_display"), "Updates Cancelled", "Event type display does not match");
        Assert.assertEquals(locationListAuditLogsResponse.jsonPath().getString("data[" + lastIndex + "].config_status_display"), "Updates Cancelled", "Config status display does not match");
        ...
        Assert.assertEquals(locationListAuditLogsResponse.jsonPath().getString("data[" + lastIndex + "].event_type"), "updates_cancelled", "Event type does not match");
        ...
        Assert.assertEquals(locationListAuditLogsResponse.jsonPath().getString("data[" + lastIndex + "].config_status"), "updates_cancelled", "Config status does not match");
        ...
        // validating app version and policy status before heartbeat api is sent
        ...
        Assert.assertEquals(locationListResponse.jsonPath().get("data[0].config_status_display"), "Updates Cancelled", "Policy Status is not correct on Provisioned Location");
        ...
        Assert.assertEquals(secondPolicyName, locationListResponse.jsonPath().get("data[0].policy.name"), "Policy is not correct on Provisioned Location");
        Assert.assertEquals(locationListResponse.jsonPath().get("data[0].config_status_display"), "Updates Cancelled", "Policy Status is not correct on Provisioned Location");
        Assert.assertEquals(locationListResponse.jsonPath().get("data[0].store_app_version"), dataSet.get("packageVersion"), "Package Name is not correct on Provisioned Location");

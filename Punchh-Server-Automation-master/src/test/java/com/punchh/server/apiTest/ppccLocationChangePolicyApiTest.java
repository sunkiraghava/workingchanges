 ...
 * @brief This class contains API test cases for the change policy API.
 * @fileName ppccLocationChangePolicyApiTest.java
 */
...
public class ppccLocationChangePolicyApiTest {
    static Logger logger = LogManager.getLogger(ppccLocationChangePolicyApiTest.class);
    ...
    @Test(description =  "SQ-T6585 Verify the flow of change policy api for a location", groups = { "regression" }, priority = 1)
    public void SQ_T6585_verifyTheFlowOfChangePolicyAPIForALocation() throws Exception {
       ...
       // getting the policy ids
        ...
        String getUtcIsoTimestampPlusFiveMinutes = CreateDateTime.generateTimestampFiveMinutesAhead();
        ...
        String queryParam = "?location_id=" + locationId + "&search=&event_type=Reprovisioned";
        ...
        Assert.assertEquals(locationListAuditLogsResponse.jsonPath().getString("data[" + lastIndex + "].event_type_display"), "Reprovisioned", "Event type display does not match");
        ...
        Assert.assertEquals(locationListAuditLogsResponse.jsonPath().getString("data[" + lastIndex + "].event_type"), "reprovisioned", "Event type does not match");
        ...
        String configFetchAfter = (String) locationListResponse.jsonPath().get("data[0].config_fetch_after");
        Assert.assertEquals(configFetchAfter.substring(0, 16), getUtcIsoTimestampPlusFiveMinutes, "Policy Status is not correct on Provisioned Location");
        ...
        Assert.assertNull(isPolicyChanged,"Is Policy Changed returned in header as not matching with policy");
        Assert.assertEquals(isPackageChanged,"True","Is Package Changed returned in header as not matching with package");
        TestListeners.extentTest.get().pass("Verified headers in heartbeat api after change policy on provisioned location at 1st time without assigning package and policy details");
        ...
        TestListeners.extentTest.get().pass("Verified heartbeat api on Change Policy for provisioned location with assigned package and policy");
        ...
        Assert.assertEquals(locationListResponse.getStatusCode(), ApiConstants.HTTP_STATUS_OK,
                "Status code 200 did not match for location list API");
        Assert.assertEquals(secondPolicyName, locationListResponse.jsonPath().get("data[0].policy.name"), "Policy is not correct on Change Policy After Sync");
        Assert.assertEquals(locationListResponse.jsonPath().get("data[0].config_status_display"), "Synced", "Policy Status is not correct on Change Policy After Sync");
        Assert.assertEquals(locationListResponse.jsonPath().get("data[0].store_app_version"), dataSet.get("packageVersion"), "Package Name is not correct on Change Policy After Sync");
        Assert.assertEquals(locationListResponse.jsonPath().get("data[0].package_status_display"),"Synced", "Package Status is not correct on Change Policy After Sync");

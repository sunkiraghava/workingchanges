 ...
 * @brief This class contains API test cases for the initiate update action API.
 * @fileName ppccLocationinitiateUpdateApiTest.java
 */
...
public class ppccLocationinitiateUpdateApiTest {
    static Logger logger = LogManager.getLogger(ppccLocationinitiateUpdateApiTest.class);
    ...
    @Test(description =  "SQ-T6185 Verify the flow of initiate update api for a location", groups = { "regression" }, priority = 1)
    public void SQ_T6587_verifyTheFlowOfInitiateUpdateAPIForALocation() throws Exception {
       ...
       String firstPolicyName = pageObj.ppccUtilities().createPolicy(token, status);
       String secondPolicyName = pageObj.ppccUtilities().createPolicy(token, status);

       // getting the policy ids
        int firstPolicyId = pageObj.ppccUtilities().getPolicyId(firstPolicyName, token);
        int secondPolicyId = pageObj.ppccUtilities().getPolicyId(secondPolicyName, token);
        ...
        Response provisionApiResponse = pageObj.endpoints().provisionApi(token, firstPolicyId, locationIds, dataSet.get("packageVersionId"));
        ...
        logger.info("reprovision the location");
        Response reprovisionApiResponse = pageObj.endpoints().reprovision(token, locationIds, secondPolicyId);
        Assert.assertEquals(reprovisionApiResponse.getStatusCode(), ApiConstants.HTTP_STATUS_OK,
                "Status code 200 did not match for reprovisioning API");
        TestListeners.extentTest.get().pass("reprovision location is giving 200 status code");

        // cancel update the location
        logger.info("Cancel update the location");
        Response cancelUpdateApiResponse = pageObj.endpoints().cancelUpdate(token, locationIds);
        Assert.assertEquals(cancelUpdateApiResponse.getStatusCode(), ApiConstants.HTTP_STATUS_OK,
                "Status code 200 did not match for cancel update API");
        TestListeners.extentTest.get().pass("cancel update location is giving 200 status code");

        // initiate update the location
        logger.info("initiate update the location");
        Response initiateUpdateApiResponse = pageObj.endpoints().initiateUpdate(token, locationIds);
        Assert.assertEquals(initiateUpdateApiResponse.getStatusCode(), ApiConstants.HTTP_STATUS_OK,
                "Status code 200 did not match for initiate update API");
        TestListeners.extentTest.get().pass("initiate update location is giving 200 status code");

        // checking audit logs
        logger.info("Checking audit logs for initiate update action");
        ...
        Assert.assertEquals(locationListAuditLogsResponse.jsonPath().getString("data[" + lastIndex + "].event_type_display"), "Initiate Update", "Event type display does not match");
        Assert.assertEquals(locationListAuditLogsResponse.jsonPath().getString("data[" + lastIndex + "].config_status_display"), "Pending Update", "Config status display does not match");
        Assert.assertEquals(locationListAuditLogsResponse.jsonPath().getString("data[" + lastIndex + "].package_status_display"), "Ready to Install", "Package status display does not match");
        ...
        Assert.assertEquals(locationListAuditLogsResponse.jsonPath().getString("data[" + lastIndex + "].event_type"), "initiate_update", "Event type does not match");
        ...
        Assert.assertEquals(locationListAuditLogsResponse.jsonPath().getString("data[" + lastIndex + "].config_status"), "pending_update", "Config status does not match");
        Assert.assertEquals(locationListAuditLogsResponse.jsonPath().getString("data[" + lastIndex + "].package_status"), "ready_to_install", "Package status does not match");
        Assert.assertEquals(locationListAuditLogsResponse.jsonPath().getString("data[" + lastIndex + "].location_package_version_id"), dataSet.get("packageVersionId"), "Location package version ID does not match");
        ...
        Assert.assertEquals(secondPolicyName, locationListResponse.jsonPath().get("data[0].policy.name"), "Policy is not correct on Provisioned Location");
        Assert.assertEquals(locationListResponse.jsonPath().get("data[0].config_status_display"), "Pending Update", "Policy Status is not correct on Provisioned Location");
        ...
        Assert.assertEquals(locationListResponse.jsonPath().get("data[0].package_status_display"),"Ready to Install", "Package Status is not correct on Provisioned Location");
        ...
        // Change Policy -> validating the app version and policy status after syncing the location with heartbeat api
        queryParam = "?is_provisioned=True&search=" + storeName;
        locationListResponse = pageObj.endpoints().getLocationList(token, queryParam);
        Assert.assertEquals(locationListResponse.getStatusCode(), ApiConstants.HTTP_STATUS_OK,
                "Status code 200 did not match for location list API");
        Assert.assertEquals(secondPolicyName, locationListResponse.jsonPath().get("data[0].policy.name"), "Policy is not correct on Provisioned Location After Sync");
        Assert.assertEquals(locationListResponse.jsonPath().get("data[0].config_status_display"), "Synced", "Policy Status is not correct on Provisioned Location After Sync");
        Assert.assertEquals(locationListResponse.jsonPath().get("data[0].store_app_version"), dataSet.get("packageVersion"), "Package Name is not correct on Provisioned Location After Sync");
        Assert.assertEquals(locationListResponse.jsonPath().get("data[0].package_status_display"),"Synced", "Package Status is not correct on Provisioned Location After Sync");
       ...
       // deleting policies
        pageObj.ppccUtilities().deletePolicy(firstPolicyId, token);
        pageObj.ppccUtilities().deletePolicy(secondPolicyId, token);
    }

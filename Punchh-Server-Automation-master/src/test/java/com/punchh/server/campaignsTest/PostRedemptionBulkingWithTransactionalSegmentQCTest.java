...
public class PostRedemptionBulkingWithTransactionalSegmentQCTest {
	private static Logger logger = LogManager.getLogger(PostRedemptionBulkingWithTransactionalSegmentQCTest.class);
...
//bulking for signup and post redemption is not working on production so  we can disable it for some time atleast for this quarter
	// Author: Amit Kumar
	@Test(description = "SQ-T6385 Verify gifting from post redemption offer campaign with transactional segment having QC attached finite gifting =1 when bulking is enable disable", dataProvider = "TestDataProvider", priority = 0)
	@Owner(name = "Amit Kumar")
	public void T6385_verifyPostRedeemptionCampaignWithTransactionalSegmentQCWhenBulkingIsDisabledEnabled(
			String flagStatus, int waitTime) throws InterruptedException, ParseException {

		// User register/signup using API2 Signup
		userEmail = pageObj.iframeSingUpPage().generateEmail();
		Response signUpResponse = pageObj.endpoints().Api2SignUpwithFavLocation(userEmail, dataSet.get("client"),
				dataSet.get("secret"), dataSet.get("favLocationId"));
		...
		pageObj.signupcampaignpage().setLocationApplicability("Oregon - 1235");
		...
		Assert.assertTrue(rewardGiftCount == 1, "reward gifted count did not matched");
		TestListeners.extentTest.get()
				.pass("reward gifting happenend twice matched with campaign frequency limit on timeline");

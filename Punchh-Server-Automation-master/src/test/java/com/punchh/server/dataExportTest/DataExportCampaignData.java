...
public class DataExportCampaignData {
	static Logger logger = LogManager.getLogger(DataExportCampaignData.class);
	...
	@Test(description = "DP1-T220 Verify the campaign export with the new field Root Campaign End Date", groups = "Regression", priority = 0)
	public void T220_VerifyDataCampaignExportWithRootCampaignEndDateField() throws InterruptedException {
		...
		String exportName = CreateDateTime.getUniqueString("T220_CampaignDataExportWithRootCampaignEndDateField");
...
//		pageObj.instanceDashboardPage().loginToInstance();
		...
		boolean flag = pageObj.dataExportPage().checkLabelPresent(exportField, "Root Campaign End Date",
				"Available Fields");
		Assert.assertTrue(flag, "User ID label is not present");
		logger.info("Root Campaign End Date label is present");
		TestListeners.extentTest.get().pass("Root Campaign End Date label is present");
		driver.navigate().refresh();

		boolean flag1 = pageObj.dataExportPage().checkLabelPresent(exportField, "Segment definition ID",
				"Selected Fields");
		Assert.assertTrue(flag1, "Segment definition ID label is not present");
		logger.info("Segment definition ID label is present");
		TestListeners.extentTest.get().pass("Segment definition ID label is present");
		driver.navigate().refresh();

		boolean flag2 = pageObj.dataExportPage().checkLabelPresent(exportField, "Segment name", "Selected Fields");
		Assert.assertTrue(flag2, "Segment name label is not present");
		logger.info("Segment name label is present");
		TestListeners.extentTest.get().info("Segment name ID label is present");
		driver.navigate().refresh();

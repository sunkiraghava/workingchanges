...
public class DataExportLocationsDataAllLocationTest {
	static Logger logger = LogManager.getLogger(DataExportLocationsDataAllLocationTest.class);
	...
	@Test(description = "SQ-T4429 Location stats export and Payment report option cleanup: Confirm that the removal of these options does not impact the payment and location data sets under the data export schedule, ensuring compatibility with other data sets. ||"
	...
	public void T4429_verifyLocationDataExport() throws Exception {
		...
		String exportName = CreateDateTime.getUniqueString("T4429_AutoDataExport");
		...
		boolean flag = pageObj.schedulePage().verifySchedulePresent("Location stats export Schedule");
		Assert.assertFalse(flag, "Location Stats Export Schedule is Visible [condition fail]");
		logger.info("Location Stats Export Schedule is not Visible [condition pass]");
		TestListeners.extentTest.get().pass("Location Stats Export Schedule is not Visible [condition pass]");

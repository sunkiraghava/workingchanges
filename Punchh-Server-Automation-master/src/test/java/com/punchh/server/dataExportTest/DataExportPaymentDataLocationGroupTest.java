...
public class DataExportPaymentDataLocationGroupTest {
	static Logger logger = LogManager.getLogger(DataExportPaymentDataLocationGroupTest.class);
	...
	@Test(description = "SQ-T4323 Payment dataSet export: Existing and New schedule: Verify that the payment export runs successfully with all other data sets, including location, location group, and single location for new data export.", groups = "Regression", priority = 0)
	public void T4323_verifyPaymentDataExportLocationGrp() throws Exception {
		...
		String exportName = CreateDateTime.getUniqueString("T4323_AutoDataExportPaymentPunchh");
		...
		List<String> fieldList = pageObj.dataExportPage().createNewDataExportWithCustomData(exportName, exportField,
				"select all", "Current Week", "location", dataSet.get("locationGrpName"), "");

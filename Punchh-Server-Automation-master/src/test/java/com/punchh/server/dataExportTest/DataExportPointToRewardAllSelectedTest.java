...
public class DataExportPointToRewardAllSelectedTest {
	static Logger logger = LogManager.getLogger(DataExportPointToRewardAllSelectedTest.class);
	...
	@Test(description = "SQ-T5340 Run All Data Export with all field selected on Point to Reward Business (Objective-2)" +
			"SQ-T2679 : Coupon Redemption Export : To Verify the attribute added in Coupon Redemption Export." +
			"SQ-T3459 : User Subscription Data : Verify new export being added for User Subscription Data for Data export.")
	public void T5340_verifyDataExportPointToRewardAllFieldSelected() throws InterruptedException {
		...
		String exportName = CreateDateTime.getUniqueString("T5340_AutoDataExport_AllFieldSelected");
		...
		pageObj.dataExportPage().setDataExportLocation("location", dataSet.get("locationGrpName"));
		...
		// verify particular value is present in fieldMap or not
		List<String> coupon_redemption_fieldList = fieldMap.get("coupon_redemption");
		Assert.assertTrue(coupon_redemption_fieldList.contains("Coupon Code"), 
				"Coupon Code is not present in coupon_redemption field list") ;
		logger.info("Coupon Code is present in coupon_redemption field list");
		TestListeners.extentTest.get().info("Coupon Code is present in coupon_redemption field list");

		List<String> user_subscription_fieldList = fieldMap.get("user_subscription");
		Assert.assertEquals(user_subscription_fieldList, dataSet.get("expectedUserSubscriptionFields")) ;
		logger.info("Successfully verified user_subscription field list");
		TestListeners.extentTest.get().info("Successfully verified user_subscription field list");

...
public class DataExportUserIdLabelRedemptionDataTest {
	static Logger logger = LogManager.getLogger(DataExportUserIdLabelRedemptionDataTest.class);
	...
	@Test(description = "SQ-T2695 Verify 'User Id' is being populated as Selected fields || "
			+ "SQ-T2676 Change in Date format consistency for Guest and Redemption Data Export", groups = "Regression", priority = 0)
	public void T2695_verifyUserIdLabeltDataExportPart_RedemptionData() throws InterruptedException {
		pageObj.sidekiqPage().sidekiqCheck(baseUrl);

		// Date verification
		String pastDate = CreateDateTime.getSundayDateOfCurrentWeek() + " 12:00 AM IST";
		String futureDate = CreateDateTime.getSaturdayDateOfCurrentWeek() + " 11:59 PM IST";

		String exportField = dataSet.get("exportField");
		logger.info("== Data export validation test for " + exportField + " ==");
		String exportName = CreateDateTime.getUniqueString("T2695_RedemptionData_AutoDataExport");
		...
		pageObj.dataExportPage().verifyDateInDataExportReport(fileName, fieldList, pastDate, futureDate);

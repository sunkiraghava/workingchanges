 ...
 * @brief This class contains UI test cases for the POS Control Center > Location > Change Policy functionality.
 * @fileName ppccLocationChangePolicyTest.java
 */
...
public class ppccLocationChangePolicyTest {
    static Logger logger = LogManager.getLogger(ppccLocationChangePolicyTest.class);
    ...
    @Test(description = "SQ-T6198 Verify the flow of change policy a location" +
            "SQ-T6192 Verify the columns of audit log of Change Policy Event.")
    @Owner(name = "Kalpana")
    public void T6198_verifyTheFlowOfChangePolicyALocation() throws Exception{
        ...
        // After Change Policy, validating UI column app version and policy status before heartbeat api is sent
        String policyNameBeforeSync=pageObj.ppccLocationPage().getPolicyNameofLocation();
        Assert.assertEquals(changePolicy, policyNameBeforeSync, "Policy Name is not correct on Provisioned Location after Change Policy");
        pageObj.utils().logPass("Policy Name is correct on Provisioned Location after Change Policy");
        String policyStatusBeforeSync=pageObj.ppccLocationPage().getPolicyStatusOfLocation();
        Assert.assertEquals(policyStatusBeforeSync, "Validating", "Policy Status is not correct on Provisioned Location after Change Policy");
        pageObj.utils().logPass("Policy Status is correct after provisioning Location after Change Policy");
        String packageNameBeforeSync=pageObj.ppccLocationPage().getPackageNameofLocation();
        Assert.assertEquals(packageNameBeforeSync, "--","Package Name is not correct on Provisioned Location after Change Policy");
        pageObj.utils().logPass("Package Name is correct after provisioning Location after Change Policy");
        String packageStatusBeforeSync=pageObj.ppccLocationPage().getPackageStatusofLocation();
        Assert.assertEquals(packageStatusBeforeSync,"Ready to Install", "Package Status is not correct on Provisioned Location after Change Policy");
        pageObj.utils().logPass("Package Status is correct after provisioning Location after Change Policy");
        ...
        pageObj.utils().logPass("Verified headers in heartbeat api after change policy on provisioned location at 1st time without assigning package and policy details");
        ...
        pageObj.utils().logPass("Verified heartbeat api on Change Policy for provisioned location with assigned package and policy");
        pageObj.ppccLocationPage().searchLocationsInProvisionedList(dataSet.get("locationName"));

        // Change Policy -> validating the app version and policy status column after syncing the location with heartbeat api
        String policyNameAfterSync=pageObj.ppccLocationPage().getPolicyNameofLocation();
        Assert.assertEquals(changePolicy, policyNameAfterSync, "Policy is not correct on Change Policy After Sync");
        pageObj.utils().logPass("Policy Name is correct on Change Policy after Sync");
        String policyStatusAfterSync=pageObj.ppccLocationPage().getPolicyStatusOfLocation();
        Assert.assertEquals(policyStatusAfterSync, "Synced", "Policy Status is not correct on Change Policy After Sync");
        pageObj.utils().logPass("Policy Status is correct on Change Policy After Sync");
        String packageNameAfterSync=pageObj.ppccLocationPage().getPackageNameofLocation();
        Assert.assertEquals(dataSet.get("packageVersion"),packageNameAfterSync,"Package Name is not correct on Change Policy After Sync");
        pageObj.utils().logPass("Package Name is correct on Change Policy After Sync");
        String packageStatusAfterSync=pageObj.ppccLocationPage().getPackageStatusofLocation();
        Assert.assertEquals(packageStatusAfterSync,"Synced", "Package Status is not correct on Change Policy After Sync");
        pageObj.utils().logPass("Package Status is correct on Change Policy After Sync");
        pageObj.utils().logPass("E2E flow of Change Policy a Location is verified successfully");
        ...
        Assert.assertEquals(eventType, "Reprovisioned", "EventType do not exists in audit logs");
        ...
        //verify the audit log display details for Provision Event
        ...
        Assert.assertEquals(eventTypeTimeAuditLogDetails, "Reprovisioned", "Event Type is not correct in audit logs display details");
        ...
        Assert.assertEquals(policyIdChangeLog.get("newValue"), String.valueOf(changepolicyId), "New value  Policy ID mismatch");
        ...
        Assert.assertEquals(policyNameChangeLog.get("newValue"), changePolicy, "New value Policy Name mismatch");
        ...
        pageObj.utils().logPass("Location is deprovisioned successfully After change Policy");

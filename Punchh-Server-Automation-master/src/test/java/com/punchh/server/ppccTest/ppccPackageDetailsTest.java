 ...
 * @fileName ppccPackageDetailsTest.java
 */
...
public class ppccPackageDetailsTest {
	...
	public void T6199_verifyTheFlowOfRemoteUpgradeALocation() throws Exception {
		...
		Assert.assertEquals(createPolicyResponse.jsonPath().get("data"),
				"Policy `" + createdPolicy + "` created successfully", "Messages do not match");
		Assert.assertEquals(createPolicyResponse.jsonPath().get("metadata"), Collections.emptyMap(),
				"Metadata has some value");
		Assert.assertEquals(createPolicyResponse.jsonPath().getList("errors"), Collections.emptyList(),
				"Response has some errors");
		...
		// validating UI column app version and policy status before heartbeat api is
		// sent
		...
		Assert.assertEquals(policyStatusBeforeSync, "Ready to Install",
				"Policy Status is not correct on Provisioned Location");
		...
		Assert.assertEquals(packageStatusBeforeSync, "Pending Update",
				"Package Status is not correct on Provisioned Location");
		pageObj.utils().logPass("Package Status is correct after provisioning Location");

		// Sending heartbeat api without matching package details and policy last
		// updated at
		Response heartbeatApiResponse = pageObj.endpoints().heartbeatApi(dataSet.get("locationKey"),
				dataSet.get("packageVersionRandom"), dataSet.get("packageVersionIdRandom"),
				dataSet.get("lastUpdatedAtRandom"));
		...
		Assert.assertEquals(isPolicyChanged, "True",
				"Is Policy Changed returned in header as not matching with policy");
		Assert.assertEquals(isPackageChanged, "True",
				"Is Package Changed returned in header as not matching with package");
		pageObj.utils().logPass(
				"Verified headers in heartbeat api for provisioned location at 1st time without assigning package and policy details");
		...
		// sending heartbeat api with matching package details and policy details to
		// sync the location
		Response heartbeatApiResponseForSync = pageObj.endpoints().heartbeatApi(dataSet.get("locationKey"),
				dataSet.get("packageVersionforRemoteUpgrade"), dataSet.get("packageVersionIdforRemoteUpgrade"),
				lastUpdatedAt);
		...
		// validating the app version and policy status column after syncing the
		// location with heartbeat api
		String policyNameAfterSync = pageObj.ppccLocationPage().getPolicyNameofLocation();
		Assert.assertEquals(createdPolicy, policyNameAfterSync,
				"Policy is not correct on Provisioned Location After Sync");
		...
		Assert.assertEquals(policyStatusAfterSync, "Synced",
				"Policy Status is not correct on Provisioned Location After Sync");
		...
		Assert.assertEquals(dataSet.get("packageVersionforRemoteUpgrade"), packageNameAfterSync,
				"Package Name is not correct on Provisioned Location After Sync");
		...
		Assert.assertEquals(packageStatusAfterSync, "Synced",
				"Package Status is not correct on Provisioned Location After Sync");
		...
		String storeNameAuditLogDetails = pageObj.ppccLocationAuditLogPage()
				.getStoreNameInLocationAuditLogDisplayDetails();
		Assert.assertEquals(storeNameAuditLogDetails, dataSet.get("locationName"),
				"Store Name is not correct in audit logs display details");

		String statusAuditLogDetails = pageObj.ppccLocationAuditLogPage().getStatusInLocationAuditLogDisplayDetails();
		Assert.assertEquals(statusAuditLogDetails, "Ready to Install",
				"Status is not correct in audit logs display details");

		String eventDateTimeAuditLogDetails = pageObj.ppccLocationAuditLogPage()
				.getEventDateTimeInLocationAuditLogDisplayDetails();
		Assert.assertEquals(eventDateTimeAuditLogDetails, eventDateTime,
				"Event Date Time is not correct in audit logs display details");
		...
		Assert.assertEquals(eventTypeTimeAuditLogDetails, "Remote Upgrade",
		...
		Map<String, String> packageStatusChangeLog = pageObj.ppccLocationAuditLogPage()
				.getFieldValuesInAuditDetailsChangeLog("Package Status");
		Assert.assertEquals(packageStatusChangeLog.get("oldValue"), "Ready to Install",
				"Old value Package Status mismatch");
		Assert.assertEquals(packageStatusChangeLog.get("newValue"), "Pending Update",
				"New value  Package Status mismatch");
		pageObj.utils().logPass("Package Status is verified successfully");

		Map<String, String> packageVersionChangeLog = pageObj.ppccLocationAuditLogPage()
				.getFieldValuesInAuditDetailsChangeLog("Package Version");
		Assert.assertEquals(packageVersionChangeLog.get("oldValue"), dataSet.get("packageVersion"),
				"Old value Package Version mismatch");
		Assert.assertEquals(packageVersionChangeLog.get("newValue"), dataSet.get("packageVersionforRemoteUpgrade"),
				"New value Package Version mismatch");
		pageObj.utils().logPass("Package Version is verified successfully");

		Map<String, String> packageVersionIdChangeLog = pageObj.ppccLocationAuditLogPage()
				.getFieldValuesInAuditDetailsChangeLog("Package Version ID");
		Assert.assertEquals(packageVersionIdChangeLog.get("oldValue"), dataSet.get("packageVersionId"),
				"Old value Package Version ID mismatch");
		Assert.assertEquals(packageVersionIdChangeLog.get("newValue"), dataSet.get("packageVersionIdforRemoteUpgrade"),
				"New value  Package Version ID mismatch");
		...
		Assert.assertEquals(deleteApiResponse.getStatusCode(), ApiConstants.HTTP_STATUS_OK,
				"Status code 200 did not match for delete policy API");
		Assert.assertEquals(deleteApiResponse.jsonPath().getList("errors"), Collections.emptyList(),
				"Response has some errors");

...
public class ppccPolicyAuditLogsTest {
	...
	public void T6187_verifyTheFlowOfDeprovisionALocation() throws Exception {
		...
		Assert.assertEquals(fetchConfigResponse.jsonPath().getString("errors[0]"),
				"Location is not provisioned with a policy.", "De-provision is not successful");
		logger.info(fetchConfigResponse.jsonPath().getString("errors[0]"));
		...
		Response heartbeatApiResponseForSync = pageObj.endpoints().heartbeatApi(dataSet.get("locationKey"),
				dataSet.get("packageVersion"), dataSet.get("packageVersionId"), dataSet.get("lastUpdatedAtRandom"));
		...
		Assert.assertEquals(heartbeatApiResponseForSync.jsonPath().getString("errors[0]"),
				"Location is not provisioned with a policy.", "De-provision is not successful");
		logger.info(fetchConfigResponse.jsonPath().getString("errors[0]"));
		...
		String policyIdAuditLogDetails = pageObj.ppccLocationAuditLogPage()
				.getPolicyIdInLocationAuditLogDisplayDetails();
		...
		Assert.assertEquals(statusAuditLogDetails, "Unprovisioned",
		...
		Assert.assertEquals(eventTypeTimeAuditLogDetails, "Deprovisioned",
				"Event Type is not correct in audit logs display details");

		// Access values from changelog for deprovision event
		Map<String, String> posTypeChangeLog = pageObj.ppccLocationAuditLogPage()
				.getFieldValuesInAuditDetailsChangeLog("POS Type");
		...
		Map<String, String> policyIdChangeLog = pageObj.ppccLocationAuditLogPage()
				.getFieldValuesInAuditDetailsChangeLog("Policy ID");
		Assert.assertEquals(policyIdChangeLog.get("oldValue"), String.valueOf(policyId),
				"Old value Policy ID mismatch");
		...
		Map<String, String> policyNameChangeLog = pageObj.ppccLocationAuditLogPage()
				.getFieldValuesInAuditDetailsChangeLog("Policy Name");
		...
		Map<String, String> configStatusChangeLog = pageObj.ppccLocationAuditLogPage()
				.getFieldValuesInAuditDetailsChangeLog("Config Status");
		Assert.assertEquals(configStatusChangeLog.get("oldValue"), "Ready to Install",
				"Old value Config Status mismatch");
		...
		Assert.assertEquals(packageStatusChangeLog.get("newValue"), "Unprovisioned",
				"New value  Package Status mismatch");
		pageObj.utils().logPass("Package Status is verified successfully");

		Map<String, String> remoteUpgradeChangeLog = pageObj.ppccLocationAuditLogPage()
				.getFieldValuesInAuditDetailsChangeLog("Remote Upgrade");
		...
		Assert.assertEquals(packageVersionIdChangeLog.get("newValue"), "\"\"",

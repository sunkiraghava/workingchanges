...
public class ContainsReceiptTagsSegmentsTest {
	static Logger logger = LogManager.getLogger(ContainsReceiptTagsSegmentsTest.class);
	...
	@Test(description = "Create ContainsReceiptTag segments using segment creation API and validate their count in UI")
	public void SegmentBuilderClausesTest() throws Exception {
		String Username1 = dataSet.get("Username1");
		List<String> segmentsNames = new ArrayList<>();
		List<String> segmentsIDS = new ArrayList<>();
		utils.logit("Starting ContainsReceiptTag segment creation and validation using builder clause");
		Set<Object> keys = prop.keySet();
		String prefixString = "ContainsReceiptTag";
		for (Object k : keys) {
			String key = (String) k;
			if (key.startsWith(prefixString)) {
				value = prop.getProperty(key);
				String segmentBetaName = pageObj.segmentsBetaPage().generateSegmentBetName() + "_" + key;
				String builderClause = utils.convertToDoubleStringifiedProperties(value);
				Response segmentCreationResp = pageObj.endpoints()
						.segmentCreationUsingBuilderClause(dataSet.get("apiKey"), segmentBetaName, builderClause);
				Assert.assertEquals(segmentCreationResp.getStatusCode(), 200);
				String segmentId = segmentCreationResp.jsonPath().get("data.id").toString();
	...
	@Test(description = "Create beta segements with SQL query and validate their count in ui")
	public void SegmentBuilderClausesTestPartTwo() throws Exception {
		...
		Set<Object> keys = prop.keySet();
		String prefixString = "PartTwoContainsReceiptTag";
		int counter = 0;
		for (Object k : keys) {
			String key = (String) k;
...
//						.warning("Segment name :" + segmentsIDS.get(i) + "count is 0 hence not deleting it");
				...
				utils.logWarn("Segment deleted is: " + segmentsIDS.get(i));
			}

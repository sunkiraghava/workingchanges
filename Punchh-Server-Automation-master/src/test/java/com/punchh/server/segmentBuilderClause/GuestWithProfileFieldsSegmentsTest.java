...
public class GuestWithProfileFieldsSegmentsTest {
	static Logger logger = LogManager.getLogger(GuestWithProfileFieldsSegmentsTest.class);
	...
	public void betaSegementsGuestWithProfileFieldsTest() throws Exception {
		...
		String prefixString = "GuestWithProfileFields";
		int counter = 0;
		for (Object k : keys) {
			String key = (String) k;
		...
		for (int i = 0; i < segmentsNames.size(); i++) {
			...
			logger.info("successfully fetched segment id with query :" + getSegmentIdQuery);
			TestListeners.extentTest.get().info("successfully fetched segment id with query :" + getSegmentIdQuery);
			while (rs.next()) {
				String segId = rs.getString(1);
				String segname = rs.getString(2);
				logger.info(segId);
				TestListeners.extentTest.get().info("segment id is :" + segId);
				String segmentId = segId + " : " + segname;
				segmentsIDS.add(segmentId);
			}
		}
		logger.info(segmentsNames.size());
		TestListeners.extentTest.get().info("segments Names size " + segmentsNames.size());
		logger.info(segmentsIDS.size());
		TestListeners.extentTest.get().info("segments IDS size " + segmentsIDS.size());
		logger.info(segmentsIDS);
		TestListeners.extentTest.get().info("segments IDS " + segmentsIDS);
...
//						.warning("Segment name :" + segmentsIDS.get(i) + "count is 0 hence not deleting it");
				...
				logger.info("Verified that status of  " + Username1 + " is not present in Segment");
				TestListeners.extentTest.get()
						.pass("Verified that status of " + Username1 + " is not present in Segment");
				// Delete the segment
				logger.info("Segment name :" + segmentsIDS.get(i) + "Guest count in segment is : " + count);

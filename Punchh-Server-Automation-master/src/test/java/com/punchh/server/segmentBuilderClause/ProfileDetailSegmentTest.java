...
public class ProfileDetailSegmentTest {
	static Logger logger = LogManager.getLogger(ProfileDetailSegmentTest.class);
	...
	public void beforeMethod(Method method) {
		...
		pageObj.readData().ReadDataFromJsonFileForClientSecretKey(pageObj.readData().getJsonFilePath(run , env , "Secrets"), dataSet.get("slug"));
		dataSet.putAll(pageObj.readData().readTestData);
		logger.info(sTCName + " ==>" + dataSet);
	}

	// in cockpit>>misc config>>Enable Offer distribution from external system
	...
	public void SegmentBuilderClausesTest() throws Exception {
		...
		String prefixString = "ProfileDetails";
		int counter = 0;
		for (Object k : keys) {
			String key = (String) k;
...
//				pageObj.newSegmentHomePage().clickOnSwitchToClassicBtn();
				...
				logger.info("Verified that status of  " + segmentGuest + " is present in Segment");
				TestListeners.extentTest.get()
						.pass("Verified that status of" + segmentGuest + " is present in Segment");
				TestListeners.extentTest.get()
						.info("Segment name :" + segmentsIDS.get(i) + "Guest count in segment is : " + count);
				int rs = stmt.executeUpdate(query);
	...
	public void SegmentBuilderClausesTestPartTwo() throws Exception {
		...
		List<String> segmentQueryList = new ArrayList<String>();
		Set<Object> keys = prop.keySet();
		String prefixString = "PartTwoProfileDetails";
		int counter = 0;
		for (Object k : keys) {
			String key = (String) k;

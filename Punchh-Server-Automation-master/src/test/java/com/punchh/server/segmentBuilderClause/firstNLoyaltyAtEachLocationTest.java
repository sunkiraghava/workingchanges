...
public class firstNLoyaltyAtEachLocationTest {
	static Logger logger = LogManager.getLogger(firstNLoyaltyAtEachLocationTest.class);
	...
	public void betaSegementsFirstNLoyaltyAtEachLocationTest() throws Exception {
		...
		String prefixString = "no_checkin_in_x_duration";
		int counter = 0;
		for (Object k : keys) {
			String key = (String) k;
...
//				pageObj.newSegmentHomePage().clickOnSwitchToClassicBtn();
				...
				logger.info("Verified that user " + segmentGuest + " is present in Segment");
				TestListeners.extentTest.get().pass("Verified that user " + segmentGuest + " is present in Segment");
